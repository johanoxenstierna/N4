...
class Sh_2_info(ShInfoAbstract):
    ...
    def __init__(_s, pulse, top_point):
        ...
        '''Basically they will only be plotted once so they will be unavailable for 99% of init_frames, 
        ...
        if P.A_SPS:
            ...
            _s.sps_gi_init_frames = sps_init_frames
    ...
    # def distribute_pulse_for_ls(_s, pulse):
    #     """Each of them has a starting point. """
    #
    #     lif0 = [pulse[0]]
    #     # lif0_start = pulse[0]
    #     lif1 = [pulse[1]]
    #     # lif1_start = pulse[1]  #len(pulse) // 2]
    #
    #     # for init_frame in pulse:
    #     #     id = random.randint(0, 1)
    #     #     if id == 0:
    #     #         lif0.append(init_frame)
    #     #     elif id == 1:
    #     #         lif1.append(init_frame)
    #
    #     '''For now they are first distributed and then removed if they are too early for the given lif
    #     Should be ok since ls and sps will run out anyway. Lens need to be tuned with expl'''
    #     # lif0 = [x for x in lif0 if x >= lif0_start]
    #     # lif1 = [x for x in lif1 if x >= lif1_start]
    #
    #     return lif0, lif1
